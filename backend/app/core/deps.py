...
async def get_current_admin_cached(
    session_key: str | None = Header(None, alias="Session-Key"),
    db: AsyncSession = Depends(get_db),
) -> User:
    """Admin auth với Redis cache layer. Fallback về DB nếu cache miss."""
    if not session_key:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="Thiếu Session-Key",
    ...
    # Try Redis cache
    if cache.enabled:
        cached = await cache.get_session(session_key)
        if cached:
            result = await db.execute(
                select(User).options(joinedload(User.role)).where(User.user_id == cached["user_id"])
            )
    ...
    if cache.enabled:
        await cache.set_session(session_key, {
            "user_id": str(user.user_id),
            "role_name": user.role.role_name,
        }, settings.SESSION_CACHE_TTL_HOURS * 3600)
